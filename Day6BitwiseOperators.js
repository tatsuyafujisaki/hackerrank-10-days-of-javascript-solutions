...
2. Determine when `a & b` is `k - 1`.
  ...
  * For `((k - 1) & b) === k - 1` to hold, `b` must be `k - 1` except one bit,
  which corresponds to `a`'s any bit of 0, to 1.
  To minimize the chance of `b` exceeding `n`, `b`'s corresponding bit must be `a`'s LSB of 0. Such `b` is `(k - 1) | k`.
